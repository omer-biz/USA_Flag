...
# (Modified for teaching purpose) (By Eyob Samuel)
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        self.mouseX = None
        self.mouseY = None
        self.key = None #From Eyob
        self.bind("<Button-1>", self._onClick)
        master.bind("<Key>", self._onKeyPress) #From Eyob
        self.height = height
        self.width = width
        self.autoflush = autoflush
        self._mouseCallback = None
        self._keyCallback=None #From Eyob
        self.trans = None
        self.closed = False
        master.lift()
        if autoflush: _root.update()
    ...
    def yUp(self):
        """Set coordinates of window to increase up the screen
        ...
        """
        # self.setCoords(0, 0, self.width-1, self.height-1)
        self.setCoords(-3, 3, self.width - 4, self.height + 2)
    ...
    def _onKeyPress(self, e): #From Eyob
        self.lastKey=e.keysym
...
class Arc(Oval):
    ...
    def __repr__(self):
        return "Arc({}, {}, {}, {})".format(self.p1, self.p2 , self.start, self.extent)
